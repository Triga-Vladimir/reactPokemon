...
import bg from "./imge/bg3.jpg"
import PokemonCards from "./Components/Pokemon/index";
import POKEMONS from './Components/Pokemon/Pokemon.json';
...
function App() {
  return (
    <>
      ...
      <Layout title='This is title'   urlBg={bg}>
        <p>
      ...
      <Layout title='This is title'   colorBg='lime'>
        <div className='flex'>
          {
            POKEMONS.map(({key, name, img, type, id, values}) => <PokemonCards key={id} name={name} img={img} id={id} type={type} values={values} />)
          }
        </div>
      </Layout>
